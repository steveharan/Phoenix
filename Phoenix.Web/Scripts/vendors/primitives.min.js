/*
 Basic Primitives orgDiagram v2.1.2
...
primitives.orgdiagram.BaseController.prototype._getNextLevelTreeItem=function(a,b){var c=a.actualPosition.horizontalCenter(),d,e,f,g,h=null,j=null,i,k,l,m=b?this._treeLevels.slice(a.level+1,this._treeLevels.length):this._treeLevels.slice(0,a.level).reverse();f=0;for(g=m.length;f<g;f+=1){k=m[f].treeItems;d=0;for(e=k.length;d<e;d+=1)if(l=this._visualTree(k[d]),1==l.actualVisibility)if(i=Math.abs(c-l.actualPosition.horizontalCenter()),i<j||null==j)j=i,h=l;else break;if(null!=j)break}return h};
...
a),a.connectorPlacement=5,j=this._getNewTreeItem({visibility:4}),this._visualTree.add(j.id,a.id,a),a=j;case 1:null===i.visualAggregatorId&&this._createNewVisualAggregator(i);switch(f.adviserPlacementType){case 2:this._visualTree.add(i.id,a.id,a,0);a.connectorPlacement=6;a.gravity=2;break;default:this._visualTree.add(i.id,a.id,a),a.connectorPlacement=12,a.gravity=1}1==a.actualItemType&&this._defineNavigationParent(i,a);break;case 0:h.push(a);this._defineNavigationParent(i,a);break;case 7:case 6:b=
...
1<u.length&&(t[i.id]=u)}m[i.id]=[];p[i.id]=[];this._layoutChildren(i,h,i.orgItem.childrenPlacementType,m[i.id],p[i.id])});this._orgTree.loopPostOrder(this,function(a){var a=this._treeItemsByUserId[a],b=p[a.id],d=m[a.id];if(null!=a){i=this._getAssitantsDepth(a);if(0<i&&(a.visualDepth=i+1,null!==a.visualAggregatorId&&(c=this._visualTree.node(a.visualAggregatorId),this._visualTree.hasChildren(c.id))))for(f=0;f<i-1;f+=1)c=this._createNewVisualAggregator(c);i=this._getAdvisersDepth(a);if(1<i&&(a.visualDepth+=
i-1,this._visualTree.hasChildren(a.id))){c=a;for(f=0;f<i-1;f+=1)c=this._createNewVisualAggregator(c)}k=[];f=0;for(g=b.length;f<g;f+=1){q=b[f];h=k[f]=0;for(j=q.length;h<j;h+=1)k[f]=Math.max(k[f],this._getItemDepth(q[h]))}f=0;for(g=k.length;f<g;f+=1)if(l=k[f],1<l){h=0;for(j=d[f].length;h<j;h+=1)if(n=d[f][h],this._visualTree.hasChildren(n.id))for(i=l;1<i;)n=this._createNewVisualAggregator(n),i-=1}null!=t[a.id]&&this._layoutPartners(a,t[a.id])}})};
primitives.orgdiagram.BaseController.prototype._layoutPartners=function(a,b){var c,d,e,f,g=0,h=[],j,i;d=0;for(e=b.length;d<e;d+=1)c=b[d],g=Math.max(g,c.visualDepth);d=0;for(e=b.length;d<e;d+=1){c=b[d];j=this._getLastVisualAggregator(c);for(f=this._getLastVisualAggregatorDepth(c);f<g;)j=this._createNewVisualAggregator(j),f+=1;h.push(this._getLastVisualAggregator(j).id)}i=this._getLastVisualAggregator(a);if(this._visualTree.hasChildren(i.id)){j=b[Math.floor(b.length/2)];if(1<b.length&&0==b.length%2){d=
this._visualTree.parent(j.id);e=this._findLeftSiblingIndex(j);f=this._visualTree.getChild(d.id,e).gravity||this._visualTree.getChild(d.id,e+1).gravity;j=this._getNewTreeItem({visibility:4,connectorPlacement:j.connectorPlacement&10,gravity:f});this._visualTree.add(d.id,j.id,j,e+1);for(f=1;f<g;)j=this._createNewVisualAggregator(j),j.connectorPlacement=0,f+=1}d=0;for(e=b.length;d<e;d+=1)c=b[d],this._navigationFamily.loopChildren(this,a.id,function(b,d){switch(d.actualItemType){case 5:case 2:case 4:case 1:break;
default:a.id!=c.id&&this._defineNavigationParent(c,d)}return this._navigationFamily.SKIP});j=this._getLastVisualAggregator(j);this._visualTree.moveChildren(i.id,j.id)}j.partners=h;a.partnerAggregatorId=j.id};primitives.orgdiagram.BaseController.prototype._defineNavigationParent=function(){};primitives.orgdiagram.BaseController.prototype._getLastVisualAggregatorDepth=function(a){for(var b=0;null!=a.visualAggregatorId;)a=this._visualTree.node(a.visualAggregatorId),b+=1;return b+1};
...
return b};primitives.orgdiagram.BaseController.prototype._createNewVisualAggregator=function(a){var b;b=this._getNewTreeItem({visibility:4,visualAggregatorId:a.visualAggregatorId,connectorPlacement:4==a.visibility&&0==a.connectorPlacement?0:5});this._visualTree.insert(a.id,b.id,b);a.visualAggregatorId=b.id;return b};
